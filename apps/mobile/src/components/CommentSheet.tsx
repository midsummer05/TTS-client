...
export function CommentSheet({ visible, count, videoId, onClose }: Props) {
  ...
  const query = useQuery({
    ...
    queryFn: () => api.videoComments(videoId),
    enabled: visible,
  })
      ...
      >
        ...
        >
          ...
          <View style={{ marginTop: 8, gap: 14 }}>
            {(query.data || []).map((comment, index) => (
              <View key={comment.id} style={{ flexDirection: 'row', gap: 10 }}>
                <Image
                  source={{
                    uri:
                      toMediaUrl(comment.user.avatarUrl) ||
                      `https://api.dicebear.com/9.x/thumbs/png?seed=${index}`,
                  }}
                  style={{
                    width: 34,
                    height: 34,
            ...
            {!query.data?.length ? (
              <Text style={{ color: '#777' }}>还没有评论，来抢第一条</Text>
            ) : null}
          </View>
          <View
            style={{
