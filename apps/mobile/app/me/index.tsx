...
export default function MeScreen() {
  ...
  const cart = useQuery({ queryKey: ['cart', session.user?.id], queryFn: api.cart, enabled: !!session.token })
  const orders = useQuery({ queryKey: ['orders', session.user?.id], queryFn: api.orders, enabled: !!session.token })

  useEffect(() => {
    if (!session.token && !login.isPending) login.mutate()
  }, [session.token])

  if (!session.token || cart.isLoading || orders.isLoading) return <LoadingView />
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#101014' }}>
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 96 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <Image source={{ uri: toMediaUrl(session.user?.avatarUrl) || 'https://api.dicebear.com/9.x/thumbs/png?seed=mobile-user' }} style={{ width: 68, height: 68, borderRadius: 34, backgroundColor: '#333' }} />
          <View style={{ flex: 1 }}>
        ...
        <View style={{ marginTop: 12, gap: 12 }}>
          {(orders.data || []).slice(0, 4).map((order) => (
            <TouchableOpacity key={order.id} onPress={() => router.push({ pathname: '/order/[id]', params: { id: order.id } })} style={{ padding: 14, borderRadius: 8, backgroundColor: '#1d1d25' }}>
              ...
              <Text style={{ color: '#9fa0ad', marginTop: 6 }}>{order.items[0]?.title || '商品'} · {order.status}</Text>
