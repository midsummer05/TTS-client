...
export default function MeScreen() {
  ...
  if (!session.token || cart.isLoading || orders.isLoading)
    return <LoadingView />
  if (cart.isError)
    return (
      <ErrorState
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 96 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <Image
            source={{
              uri:
                toMediaUrl(session.user?.avatarUrl) ||
                'https://api.dicebear.com/9.x/thumbs/png?seed=mobile-user',
            }}
            style={{
              width: 68,
              height: 68,
        ...
        <View style={{ marginTop: 12, gap: 12 }}>
          {(cart.data || []).slice(0, 4).map((item) => (
            ...
            >
              <Image
                source={{ uri: toMediaUrl(item.product.coverUrl) }}
                style={{
                  width: 68,
                  height: 68,
                  borderRadius: 6,
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: '#9fa0ad', marginTop: 6 }}>
                  数量 x{item.quantity}
                </Text>
                <Text
                  style={{ color: '#ff315f', marginTop: 6, fontWeight: '900' }}
                >
        ...
        <View style={{ marginTop: 12, gap: 12 }}>
          {(orders.data || []).slice(0, 4).map((order) => (
            ...
            >
              ...
              <Text style={{ color: '#9fa0ad', marginTop: 6 }}>
                {order.items[0]?.title || '商品'} · {order.status}
              </Text>
              <Text
                style={{ color: '#ff315f', marginTop: 8, fontWeight: '900' }}
              >
