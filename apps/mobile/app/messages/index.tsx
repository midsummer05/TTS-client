        ...
        <View style={{ marginTop: 18, gap: 12 }}>
          {(query.data || []).map((message, index) => (
            ...
            >
              {message.avatarUrl ? (
                <Image
                  source={{ uri: toMediaUrl(message.avatarUrl) }}
                  style={{
                    width: 42,
                    height: 42,
                    borderRadius: 21,
                ...
                <View
                  style={{
                    ...
                    height: 42,
                    borderRadius: 21,
                    backgroundColor: index % 2 === 0 ? '#ff315f' : '#3a7bff',
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}
                >
                  <Text style={{ color: '#fff', fontWeight: '900' }}>
                    {message.type === 'order' ? '单' : '互'}
                  </Text>
                </View>
              )}
              <View style={{ flex: 1 }}>
                ...
                <Text
                  style={{ color: '#c9c9d2', marginTop: 6, lineHeight: 20 }}
                >
                  {message.content}
                </Text>
              </View>
          ...
          {!query.data?.length ? (
            <Text style={{ color: '#9fa0ad' }}>暂无消息</Text>
          ) : null}
        </View>
      </ScrollView>
