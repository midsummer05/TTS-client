...
export default function CartScreen() {
  ...
  const select = useMutation({
    mutationFn: ({ id, selected }: { id: string; selected: boolean }) =>
      api.selectCart(id, selected),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cart'] }),
  })
  const remove = useMutation({
    mutationFn: api.deleteCart,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cart'] }),
  })
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f6f6f6' }}>
      ...
      >
        ...
        {query.data.map((item) => (
          ...
          >
            <TouchableOpacity
              onPress={() =>
                select.mutate({ id: item.id, selected: !item.selected })
              }
              style={{
                width: 28,
                height: 28,
                borderRadius: 14,
                backgroundColor: item.selected ? '#111' : '#ddd',
                alignItems: 'center',
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text
                style={{ color: '#e43d33', fontWeight: '800', marginTop: 6 }}
              >
      ...
      >
        <Text style={{ flex: 1, fontSize: 18, fontWeight: '800' }}>
